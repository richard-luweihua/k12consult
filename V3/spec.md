...
- 唯一按钮：`开始规划`
- 辅助文案：`5-10分钟完成`
- 交互：`开始规划` 统一跳转 ` /questionnaire `（不因登录态变化）
- 右上角登录入口
...
1. 首页“开始规划”入口统一跳转问卷，避免登录态导致错误落页。
...
10. 管理员状态看板改为“下拉选择 + 快捷标签”模式，减少纵向占用并保持筛选与数量同步。
...
- 状态看板：使用“下拉选择状态 + 快捷状态标签”紧凑交互，按所选状态动态展示案例数量与列表
...
*Version: V3.1.3 (Consolidated + Hotfix)*
