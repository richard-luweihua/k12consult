...
export function AuthProvider({ children }) {
  ...
  const signUp = async (email, password, fullName, mobile = '') => {
    if (isPlaceholderConfig()) {
      const demoUser = { id: 'demo-user', email, full_name: fullName || 'Demo User', role: 'user', mobile };
      setUser(demoUser);
      return { data: { user: demoUser }, error: null };
    }
  ...
  const requestMobileOtp = async (mobile, purpose = 'login') => {
    try {
      const response = await fetch(apiPath('/api/auth/otp/request'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      const payload = await readJson(response, '验证码发送失败');
      return { data: payload, error: null };
    } catch (error) {
      return {
        data: null,
        error: error instanceof Error ? error : new Error('验证码发送失败')
      };
    }
  };

  const verifyMobileOtp = async ({ challengeId, mobile, code }) => {
    try {
      ...
      const payload = await readJson(response, '验证码登录失败');
      setUser(payload.user);
      return { data: payload, error: null };
    } catch (error) {
      return {
        data: null,
        error: error instanceof Error ? error : new Error('验证码登录失败')
      };
    }
  };
