...
export async function ensureUserProfile({
  ...
  if (!(await hasUsersTable())) {
    return {
      ...
      role,
      mobile: typeof mobile === "string" ? mobile.trim() || null : null,
      consultant_id,
  ...
  if (error && normalizedMobile && error?.message?.includes("mobile")) {
  ...
  return {
    ...
    mobile: data?.mobile || normalizedMobile || null,
    consultant_id,
    consultantId: consultant_id
  };
}
...
export async function loginUser({ email, password }) {
  ...
  return ensureUserProfile({
    ...
    mobile: payload.user.user_metadata?.mobile || payload.user.phone || "",
...
export async function getSessionUserFromRequest(request) {
  ...
  try {
    ...
    return {
      ...session,
      ...profile,
      mobile: profile.mobile || session.mobile || null
    };
  } catch {
    return session;
  }
