...
const v2StatusOptions = [
  ...
  { key: "follow_up", label: "顾问跟进中" },
  { key: "nurturing", label: "培育池" },
  { key: "closed", label: "成交关闭" }
];
...
function filterHref(statusKey) {
  return statusKey === "all" ? "/advisor/workbench" : `/advisor/workbench?v2Status=${statusKey}`;
}
...
export default async function AdvisorPage({ searchParams }) {
  ...
  const scopedLeads = isAdminViewer
    ? leadsWithV2Status
    : leadsWithV2Status.filter((lead) => isLeadAssignedToConsultant(lead, actor.consultantKey));
  const stats = buildLeadStats(scopedLeads);
  const awaitingAssignment = scopedLeads.filter((lead) =>
    ["report_viewed", "consult_intent_submitted", "admin_following", "consult_ready_for_assignment"].includes(lead.v2Status)
  ).length;
  const newlyAssigned = scopedLeads.filter((lead) => lead.v2Status === "consult_assigned").length;
  const inProgress = scopedLeads.filter((lead) =>
    ["consult_assigned", "follow_up"].includes(lead.v2Status)
  ).length;
  const v2StatusCounts = scopedLeads.reduce(
    (acc, lead) => {
      acc.all += 1;
  ...
  const filteredLeads =
    selectedStatus === "all" ? scopedLeads : scopedLeads.filter((lead) => lead.v2Status === selectedStatus);
        ...
        <div className="hero-panel hero-panel--compact">
          <div className="hero-note">
            ...
            <div style={{ marginTop: "14px" }}>
              <strong style={{ fontSize: "2.2rem", display: "block", marginBottom: "4px" }}>{scopedLeads.length}</strong>
              <span style={{ fontSize: "0.92rem" }}>{isAdminViewer ? "当前总线索数" : "当前可见线索数"}</span>
            </div>
          </div>
